...
해결했다(2026-08-26). 이 버그로 no_issue로 잘못 방치됐던 리뷰(799
포함)는 `backend/scripts/backfill_review_classification.py`를 매장
전체(760건)에 다시 돌려 재분류했다 — 799는 `food_quality,
sentiment_conflict=true`로 정확히 잡혔고, 별점은 높지만 실제 내용에
문제가 있는 리뷰가 34건 더 있었다는 것도 이 재실행으로 확인됐다(30건은
이미 답변완료라 지나간 답글은 그대로 두고, 아직 안 끝난 4건만 새로
답글 생성해 반영).

no_issue RAG 통합 이후 실제 사용해보다가, `_SENSITIVE_TONE_OVERRIDE`
("이모지 없이 차분하고 진중하게")를 지시해도 실제 출력엔 이모지가 섞여
나오는 문제도 발견했다(2026-08-26). 원인은 [참고 예시]에 넣는
golden_examples 자체가 사장님의 실제 과거 답글이라 이모지가 섞인 경우가
많아, 프롬프트 지시문과 few-shot 예시가 서로 모순되는 신호를 준다는
것 — 텍스트 지시만으로는 few-shot 데모를 안정적으로 못 이긴다. 그래서
`generate_ai_reply`는 (1) 프롬프트에 넣는 예시 텍스트에서부터 이모지를
지워 모순 신호 자체를 없애고, (2) 최종 생성 결과에서도 한 번 더 걸러내
확정적으로 보장한다(`_strip_emoji`).

처음엔 이 톤 오버라이드(이모지 제거 포함)를 `is_sensitive`/
`sentiment_conflict` 두 조건에만 걸었으나, 실사용 중 food_quality처럼 그
둘 중 어느 것도 아닌 일반 불만 리뷰("찐사장님 말투"로 생성했는데도
이모지가 그대로 붙는 사례, 3점·닭갈비 맛 불만 리뷰)에서도 이모지가
섞여 나온다는 지적을 받았다(2026-08-31) — "위생/안전처럼 진짜 민감한
사안"과 "그냥 불만 리뷰" 둘 다 차분한 톤이어야 한다는 게 원래
의도였는데 조건이 너무 좁게 걸려 있었다. 그래서 조건을 `category !=
"no_issue"`(즉 no_issue가 아닌 모든 리뷰)로 넓혔다 — 상수 이름도
`_SENSITIVE_TONE_OVERRIDE` → `_COMPLAINT_TONE_OVERRIDE`로 바꿔 실제
적용 범위를 정확히 반영했다. no_issue(칭찬/무난) 리뷰만 페르소나
톤(이모지 포함)을 그대로 쓴다.
