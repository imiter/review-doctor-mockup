...
매출은 주문내역 화면에서 별도로 보완한다. 이때도 개별 주문을 `orders`
테이블에 행으로 저장하지는 않는다(설계 문서의 "주문내역 실데이터 연동은
범위 밖"은 여전히 유효) — 날짜별로 집계한 합계만 다른 매출 소스와 동일하게
`daily_settlements`에 upsert한다. 조사 결과 입금은 매장(브랜드)별 필터가
