...
_SEED_EXAMPLES: dict[str, tuple[str, str]] = {
    ...
    "delivery": (
        ...
        "안녕하세요, 배달이 늦어져 오래 기다리게 해드려 죄송합니다. 배달은 배달의민족에서 "
        "라이더 배차를 진행하는 부분이라 저희 가게에서 직접 조정하기는 어렵지만, 이렇게 "
        "늦어진 점 다시 한번 죄송하게 생각합니다. 지연이 확인되면 고객센터를 통해 빠른 "
        "배차를 요청하는 등 저희가 할 수 있는 부분에서 최대한 신경 쓰겠습니다.",
    ),
    "hygiene": (
