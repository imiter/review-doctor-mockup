def test_signup_creates_user_store_and_subscription(client, platforms, signup_flow):
    res = signup_flow("new@test.com", nickname="새사장", marketing_agreed=True)
...
def test_signup_leaves_phone_unset(client, platforms, db_session, signup_flow):
    """휴대폰은 더 이상 가입 단계에서 받지 않는다 — PATCH /auth/me로 나중에 추가한다."""
    from app.models import User

    signup_flow("phonecheck@test.com")
...
def test_duplicate_signup_rejected(client, platforms, signup_flow):
    ...
    # signup_flow의 사전 확인 호출(email-code)은 이제 200을 요구하므로, "이미 가입된
    # 이메일" 케이스는 email-code 단계에서부터 409가 나 fixture를 통과하지 못한다.
    # 여기서 실제로 검증하려는 건 signup() 자신의 재확인(동시 가입 레이스 대비)이므로,
    # 사전 확인 단계를 건너뛰고 최종 엔드포인트를 직접 두드린다.
...
def test_signup_final_submit_expired_email_code(client, platforms, db_session, monkeypatch):
    """설계 문서 "최종 제출 시점에 코드 만료" 요구사항: 오래 붙잡고 있다가 제출하면
    만료 메시지를 받아 프론트가 이메일 인증 단계로 되돌릴 수 있어야 한다."""
    from datetime import datetime, timedelta, timezone
...
def test_issue_code_sweeps_expired_rows_for_same_purpose(client, platforms, db_session, monkeypatch):
    """코드 발급마다 같은 purpose의 만료된 행을 쓸어내는지 확인 — 형식이 자유로운
    target으로 반복 요청하면 만료 행이 무한정 쌓일 수 있는 것을 막는다."""
    from datetime import datetime, timedelta, timezone
    ...
    now = datetime.now(timezone.utc)
    db_session.add(SignupVerification(
        target="stale-1@test.com", purpose="email", code_hash="a" * 64,
        expires_at=now - timedelta(minutes=1), attempts=0, created_at=now - timedelta(minutes=20),
    ))
    db_session.add(SignupVerification(
        target="stale-2@test.com", purpose="email", code_hash="b" * 64,
        expires_at=now - timedelta(minutes=1), attempts=0, created_at=now - timedelta(minutes=20),
    ))
    db_session.commit()

    res = client.post("/auth/signup/email-code", json={"email": "sweep@test.com"})
    assert res.status_code == 200

    remaining_stale = db_session.query(SignupVerification).filter(
        SignupVerification.target.in_(["stale-1@test.com", "stale-2@test.com"])
    ).count()
    assert remaining_stale == 0


def test_signup_consumes_verification_row_on_success(client, platforms, db_session, signup_flow):
    from app.models import SignupVerification

    signup_flow("consumed@test.com")
