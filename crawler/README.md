# 배민 카테고리 순위 실측 크롤러 + 배민 실연동 크롤 워커

메인 프로젝트(review-doctor)와 분리된 독립 도구. 이 디렉터리는 두 가지
역할을 겸한다:

1. **광고 순위 반경별 실측 크롤러** (아래 이 파일의 대부분 내용) — 내 가게
   주소(0km) + 가게 주소를 기점으로 한 반경 구간(1.5~2.5km, 2.5~3.5km)
   각 1지점, 총 3개 지점에서 배민 카테고리 순위를 실제로 측정한다. 반경
   구간은 매번 가게 주소부터 다시 계산한다(이전 지점에서 누적으로
   이어가지 않는다). 역지오코딩된 주소가 산 지번(임야)이면 실제 배달
   위치로 보기 어려워 자동으로 건너뛰고 같은 구간에서 다시 샘플링한다
   (최대 5회).
2. **배포 환경(Railway)이 위임하는 실제 배민 연동 전체의 실행 주체**
   ("배포 사이트용 워커" 절 참고) — 이 컴퓨터(맥북)가 실제로 배민에
   로그인해 리뷰·매출·정산·메뉴를 스크래핑하고, AI가 생성한 답글을
   실제로 배민에 등록하는 것도 전부 이 워커가 수행한다. 순위 크롤러는
   그중 하나의 기능일 뿐이다.
...
1. `.env.example`을 `.env`로 복사하고 값을 채운다 (카카오 REST API 키는
   ...
   **`STORE_DISPLAY_NAME`은 배민 앱 화면에 실제로 보이는 문구와 글자 하나까지
   정확히 일치해야 한다** — 스크립트가 추측하지 않으므로 앱을 직접 열어
   확인한 값을 그대로 적는다. `CATEGORY_LABEL`은 원래 "메뉴 전체보기" 탭
   라벨과 정확히 같아야 했지만(예: "구이"는 존재하지 않고 "고기"가 실제
   탭 이름), 배민 가게정보 API가 돌려주는 카테고리가 탭 라벨보다 더
   세분화된 값을 주는 경우가 실측으로 확인돼(2026-08-31, `찜·탕·찌개`
   API 값 vs `찜·탕` 탭) `navigate_to_category`(`baemin_navigator.py`)가
   지금은 정확 일치 → 탭 라벨이 접두사인 경우 → 명시적 치환표
   (`_CATEGORY_LABEL_OVERRIDES`) 순으로 매칭을 시도한다. 이 셋 중 어느
   것에도 안 걸리는 완전히 다른 이름(예: API "백반·죽·국수" ↔ 탭
   "분식")은 여전히 치환표에 직접 추가해야 한다 — 실패하면 `NAV_ERROR`로
   스킵되니, 새 카테고리에서 계속 실패하면 이 표부터 확인한다.
...
## 배포 사이트용 "워커" (배민 실연동 전체가 이 컴퓨터에서 돈다)

배포된 사이트(Railway)는 에뮬레이터도 못 돌리고, 자기 자신의 클라우드
IP에서는 배민 로그인 폼 자체가 렌더링되지 않는 게 실측 확인됐다(배민
봉 탐지로 추정). 그래서 `CRAWL_WORKER_URL`이 설정된 배포 환경에서는
**로그인이 필요한 배민 관련 작업 전부**를 이 맥북에 위임한다 — 위임은
"실행만"이고, 자격증명 암호화·DB 저장·최종 결과 반영은 항상 Railway
백엔드(또는 이 워커가 직접, DB는 공유)가 담당한다. 자세한 아키텍처는
`docs/superpowers/specs/`의 관련 설계 문서를 참고.

위임되는 서비스-간 엔드포인트 4개(`backend/app/routers/`, 전부
`X-Worker-Secret` 공유 비밀로만 인증, 사용자 JWT 아님):

| 엔드포인트 | 트리거 | 실제로 하는 일 |
|---|---|---|
| `POST /internal/baemin-login` | "가게 연결" 최초 로그인 | 배민 로그인 + 계정에 딸린 매장(브랜드) 목록 조회만. 암호화·DB 저장은 Railway가 함 |
| `POST /internal/sync-reviews` | "데이터 동기화" 버튼 + 매일 KST 04시 자동 스케줄러 | 리뷰·매출·입금·재주문율·정산 상세·우리가게클릭 성과·메뉴 정보를 전부 실제로 스크래핑해 DB에 적재. 새 리뷰는 Haiku로 분류하고, 별점 5점+무불만 리뷰는 Sonnet이 생성한 답글을 **실제로 배민에 자동 등록**까지 한다(`backend/scrapers/baemin_reply_submit.py`) |
| `POST /internal/run-crawl` | "우리가게 순위 확인" 버튼 | 이 디렉터리의 `run_crawl.py` — 반경별 실측 순위 |
| `POST /internal/apply-bid` | 우리가게클릭 CPC 입찰가 반영 | 배민에 실제 CPC 반영 후 순위 재측정까지 한 번에 |

이 중 로그인이 필요한 3개(`baemin-login`/`sync-reviews`/`apply-bid`)는
`backend/scrapers/`의 스크래퍼 모듈이 직접 처리한다 — 이 디렉터리
(`crawler/`)에는 없고 `backend/`에 있다: `baemin_auth.py`(로그인),
`baemin_reviews.py`(리뷰), `baemin_stats.py`(매출·입금·정산·주문내역,
가장 큼), `baemin_ads.py`(우리가게클릭·CPC), `baemin_menu.py`(메뉴 정보),
`baemin_reply_submit.py`(답글 실제 제출). `run_crawl.py`(이 디렉터리)만
독립 서브프로세스로 실행되고, 나머지는 워커 백엔드(`backend/`) 프로세스
안에서 직접 실행된다.
...
→ 워커 백엔드 8001 포트). 에뮬레이터/Appium은 `run-crawl`에만 필요하고
나머지 3개 엔드포인트는 Playwright(헤드리스 Chromium)만 쓴다 —
에뮬레이터가 꺼져있어도 리뷰/매출/정산 동기화는 정상 동작한다.
